...
const InvoiceReceived = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://church.boogiecoin.com/invoice-received/${editingInvoice ? editingInvoice : ''}`,
        {
  ...
  const handleDelete = async (invoiceId) => {
    ...
    try {
      const response = await fetch(`https://church.boogiecoin.com/invoice-received/${invoiceId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
